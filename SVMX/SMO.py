...
# 核函数
def kernelTrans(dataMat, rowDatamat, kTrup):
    pdb.set_trace()
...
# 定义数据结构体，用于缓存，提高运行速度
class optStruct:
    def __init__(self,dataSet,lableSet,C,toler,kTrup):
        self.dataMat = np.mat(dataSet) # m*n
        self.labelMat = np.mat(lableSet).T # m*1
        self.C = C # 惩罚参数
        self.toler = toler
        self.m = np.shape(self.dataMat)[0]
        self.alphas = np.mat(np.zeros((self.m,1))) # alpha 系数，m*1矩阵
        self.b = 0 # 偏置
        self.eCache = np.mat(np.zeros((self.m,2))) # 保存原始数据每行的预测值
        self.K = np.mat(np.zeros((self.m,self.m))) # 核转换矩阵
        pdb.set_trace()
        for i in range(self.m):
            self.K[:,i] = kernelTrans(self.dataMat, self.dataMat[i,:], kTrup)

# 计算原始数据第k项对应的误差预测
def calEk(oS, k):
    fXk = float(np.multiply(oS.alphas, oS.labelMat).T*oS.labelMat*oS.K[:,k]+oS.b)
...
def selectJ(i,oS,Ei):
    ...
    Ej = 0
    oS.eCache[i] = [1, Ei]
    validEcacheList = np.nonzero(oS.eCache[:,0].T)[0]

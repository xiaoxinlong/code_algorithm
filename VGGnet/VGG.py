...
class VGGnet(nn.Module):
    ...
    def __init__(self,features,num_classes=1000,init_weight=True):
        ...
        for i,model in enumerate(features):
            self.features.add_module(name=str(i),module=model)
...
class VGG(VGGnet):
    def __init__(self,features,num_classes=1000,init_weight=True):
        super(VGG, self).__init__(features,num_classes=1000,init_weight=True)
...
def vgg19_bn(**kwargs):
    model = VGGnet(make_layers(cfg['E'], batch_norm=True), **kwargs)
    return model
